...
#define GOMP3_MAX_SAMPLES_PER_FRAME (1152*2)
...
#endif /* GOMP3_H */
#if defined(GOMP3_IMPLEMENTATION) && !defined(_GOMP3_IMPLEMENTATION_GUARD)
#define _GOMP3_IMPLEMENTATION_GUARD
...
#define GOMP3_MIN(a, b)           ((a) > (b) ? (b) : (a))
#define GOMP3_MAX(a, b)           ((a) < (b) ? (b) : (a))

#if !defined(GOMP3_NO_SIMD)

#if !defined(GOMP3_ONLY_SIMD) && (defined(_M_X64) || defined(__x86_64__) || defined(__aarch64__) || defined(_M_ARM64))
/* x64 always have SSE2, arm64 always have neon, no need for generic code */
#define GOMP3_ONLY_SIMD
#endif /* SIMD checks... */
...
typedef __m128 f4;
#if defined(_MSC_VER) || defined(GOMP3_ONLY_SIMD)
#define gomp3_cpuid __cpuid
#else /* defined(_MSC_VER) || defined(GOMP3_ONLY_SIMD) */
static __inline__ __attribute__((always_inline)) void gomp3_cpuid(int CPUInfo[], const int InfoType)
{
#if defined(__PIC__)
...
#endif /* defined(_MSC_VER) || defined(GOMP3_ONLY_SIMD) */
static int have_simd(void)
{
#ifdef GOMP3_ONLY_SIMD
    return 1;
#else /* GOMP3_ONLY_SIMD */
    ...
    if (g_have_simd)
        goto end;
    gomp3_cpuid(CPUInfo, 0);
    g_have_simd = 1;
    if (CPUInfo[0] > 0)
    {
        gomp3_cpuid(CPUInfo, 1);
        g_have_simd = (CPUInfo[3] & (1 << 26)) + 1; /* SSE2 */
    }
end:
    return g_have_simd - 1;
#endif /* GOMP3_ONLY_SIMD */
}
...
{   /* TODO: detect neon for !GOMP3_ONLY_SIMD */
    return 1;
}
#else /* SIMD checks... */
#define HAVE_SSE 0
#define HAVE_SIMD 0
#ifdef GOMP3_ONLY_SIMD
#error GOMP3_ONLY_SIMD used, but SSE/NEON not enabled
#endif /* GOMP3_ONLY_SIMD */
#endif /* SIMD checks... */
#else /* !defined(GOMP3_NO_SIMD) */
#define HAVE_SIMD 0
#endif /* !defined(GOMP3_NO_SIMD) */

#if defined(__ARM_ARCH) && (__ARM_ARCH >= 6) && !defined(__aarch64__) && !defined(_M_ARM64)
#define HAVE_ARMV6 1
static __inline__ __attribute__((always_inline)) int32_t gomp3_clip_int16_arm(int32_t a)
{
    int32_t x = 0;
...
static const L12_subband_alloc_t *L12_subband_alloc_table(const uint8_t *hdr, L12_scale_info *sci)
{
    ...
    sci->stereo_bands = (uint8_t)GOMP3_MIN(stereo_bands, nbands);
...
#define BSPOS         ((bs_next_ptr - bs->buf)*8 - 24 + bs_sh)
    ...
    while (big_val_cnt > 0)
    {
        ...
        if (linbits)
        {
            ...
            {
                np = *sfb++ / 2;
                pairs_to_decode = GOMP3_MIN(big_val_cnt, np);
                one = *scf++;
                do
                {
                    int j, w = 5;
...
static void L3_intensity_stereo(float *left, uint8_t *ist_pos, const L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    if (gr->n_long_sfb)
    {
        max_band[0] = max_band[1] = max_band[2] = GOMP3_MAX(GOMP3_MAX(max_band[0], max_band[1]), max_band[2]);
    }
...
static int L3_restore_reservoir(mp3dec_t *h, bs_t *bs, mp3dec_scratch_t *s, int main_data_begin)
{
    ...
    int bytes_have = GOMP3_MIN(h->reserv, main_data_begin);
    memcpy(s->maindata, h->reserv_buf + GOMP3_MAX(0, h->reserv - main_data_begin), GOMP3_MIN(h->reserv, main_data_begin));
...
#endif /* HAVE_SIMD */
#ifdef GOMP3_ONLY_SIMD
    {} /* for HAVE_SIMD=1, GOMP3_ONLY_SIMD=1 case we do not need non-intrinsic "else" branch */
#else /* GOMP3_ONLY_SIMD */
    for (; k < n; k++)
    {
...
static int16_t mp3d_scale_pcm(float sample)
{
    ...
    s32 -= (s32 < 0);
    int16_t s = (int16_t)gomp3_clip_int16_arm(s32);
#else
...
#ifndef GOMP3_NONSTANDARD_BUT_LOGICAL
    if (nch == 1)
    {
        for (i = 0; i < 15*64; i += 2)
        {
...
#endif /* GOMP3_NONSTANDARD_BUT_LOGICAL */
    {
...
#endif /* GOMP3_FLOAT_OUTPUT */
#endif /* GOMP3_IMPLEMENTATION && !_GOMP3_IMPLEMENTATION_GUARD */
